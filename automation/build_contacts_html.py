...
DEFAULT_CONFIG = {
    ...
    "org_blurb": ("Every counterparty this firm has corresponded with, drawn "
                  ...
                  "who signs it off."),
    # Which folders the sweep actually read, and what the in/out counts mean.
    # Both are overridable so a narrower harvest does not describe itself as a
    # full-mailbox one.
    "source_folders": "Inbox, Sent Items and Archive",
    "counts_note_html": ("<em>In</em> and <em>out</em> count messages received "
                         "from and sent to that contact."),
}
...
<footer>
  <div class="wrap">
    ...
    <p>{built_from} ({html.escape(cfg["source_folders"])}) and
       ...
       found — not that the detail doesn't exist. {cfg["counts_note_html"]}</p>
