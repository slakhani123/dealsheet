...
def main():
    ...
    # Which folders the sweep actually read, and what the In/Out columns mean.
    # Overridable so a narrower harvest does not describe itself as a full one.
    folders = cfg.get("source_folders") or "Inbox, Sent Items and Archive"
    counts_note = cfg.get("counts_note") or (
        "'Emails In / Out' are total messages received from / sent to that "
        "contact since Jan 2025 — a rough measure of relationship activity.")
    ...
    lines = [
        ...
        (f"A weekly automated sweep reads the mailbox ({folders}), "
